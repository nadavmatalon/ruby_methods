...
def select_elements_starting_with_a(array)
	# array.select {|e| e.chars.first == "a"}
	array.select {|e| e.start_with? "a"}
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
	# array.select {|w| ["a","e","i","o","u"].include?(w.chars.first)}
	# array.select { |w| w =~ /\A[aeiou]/i }
	array.select {|w| (%w[a e i o u] & [w.chars.first]).any?}
end
...
def remove_nils_and_false_from_array(array)
	# array.reject(&:!)
	array.compact.select {|e| e != false}
end
...
def array_sort_by_last_letter_of_word(array)
	# array.sort_by { |w| w[-1] }
	array.sort_by {|w| w[w.chars.last]}
end
...
def average_of_array(array)
	# (array.inject {|total, element| total + element}.to_f / array.length).round
...
def add_together_keys_and_values(hash)
	# hash.to_a.flatten.inject(0){|total, element| total + element }
	hash.flatten.inject(:+)
end
...
def get_domain_name_from_email_address(email)
	# email.slice(email.index("@")+1..email.index(".")-1)
	email[/@(\w+)/, 1]
end
...
def titleize_a_string(string)
    string.capitalize.split.each{|e| e.capitalize! unless %w[a and the].include?(e)}.join(" ")
end
